(function (flexo) {
  ...
  flexo.promise.prototype.then = function (fulfilled, rejected) {
    var next = new flexo.promise();
    if (typeof fulfilled === "function") {
      fulfilled.__next = next;
      this._fulfilled.push(fulfilled);
    } else {
      this._fulfilled.push(next);
    }
    if (this.value) {
      on_fulfill_promise.delay(this);
    }
    if (typeof rejected === "function") {
      rejected.__next = next;
      this._rejected.push(rejected);
    } else {
      this._rejected.push(next);
    }
    if (this.reason) {
      on_reject_promise.delay(this);
    }
    return next;
  };
  ...
  flexo.promise.prototype.fulfill = function (value) {
    if (this.hasOwnProperty("value")) {
      throw "Promise was already fulfilled!";
    }
    if (this.hasOwnProperty("reason")) {
      throw "Promise was previously rejected!";
    }
    this.value = value;
    on_fulfill_promise(this);
    flexo.notify(this, "!fulfilled");
    return this;
  };

  flexo.promise.prototype.reject = function (reason) {
    if (this.hasOwnProperty("reason")) {
      throw "Promise was already rejected!";
    }
    if (this.hasOwnProperty("value")) {
      throw "Promise was previously fulfilled!";
    }
    this.reason = reason;
    on_reject_promise(this);
    flexo.notify(this, "!rejected");
    return this;
  };

  function on_fulfill_promise(promise) {
    promise._fulfilled.forEach(function (f) {
      if (f instanceof flexo.promise) {
        f.fulfill(promise.value);
      } else {
        try {
          var x = f(promise.value);
          if (x !== undefined) {
            resolve_promise(f.__next, x);
          }
        } catch (e) {
          f.__next.reject(e);
        }
  ...
  function on_reject_promise(promise) {
    promise._rejected.forEach(function (f) {
      if (f instanceof flexo.promise) {
        f.reject(promise.reason);
      } else {
        try {
          var x = f(promise.reason);
          if (x !== undefined) {
            reject_promise(f.__next, x);
          }
        } catch (e) {
          f.__next.reject(e);
        }
  ...
  function resolve_promise(promise, x) {
    if (x === promise) {
      promise.reject(new TypeError("Promise cannot be fulfilled with itself"));
    } else if (x instanceof flexo.promise) {
      if (x.hasOwnProperty("value")) {
        promise.fulfill(x.value);
      } else if (x.hasOwnProperty("reason")) {
        promise.fulfill(x.reason);
      } else {
        flexo.listen(x, "!fulfilled", function () {
          promise.fulfill(x.value);
        });
        flexo.listen(x, "!rejected", function () {
